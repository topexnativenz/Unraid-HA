// v1.0.0 · build no.16x9-7 (1920×1080 fork)
/* ════════════════════════════════════════════════════════════════════
   ...
   • Layout: canonical SLOTS measured from 1920×1080 wall tablet (forked from 1500×1000).
   • Backgrounds: /local/community/casa-luna-16x9/sky/casa-luna-<variant>.png
...
const SL = {
  nav:  { x:26, w:270, h:83, tops:[158, 247, 336, 428, 524, 613, 704, 794] },
  r_cyl:[1425, 99, 195, 311], r_stats:[1632, 147, 266, 256], r_mode:[1632, 27, 266, 112],
  r_pvtile:[1425, 415, 472, 54], r_ev:[1425, 471, 472, 54], r_cons:[1425, 527, 472, 133], r_prod:[1425, 662, 472, 133], r_events:[1425, 797, 472, 152],
  pv:[413, 621, 461, 36], pwr:[922, 621, 454, 36],
  stat_cont:[380, 669, 1023, 141],
  stat:{ y:680,h:116,w:230,xs:[396, 650, 905, 1160] },
  inv_box:[383, 823, 268, 147],
  inv_right:[652, 823, 749, 147],
  donut_c:[712, 849, 111, 114],
  invt:{ y:843,h:116,w:156,xs:[879, 1053, 1228] },
  bot:{ y:990,h:81,w:233,xs:[200, 457, 714, 972, 1229, 1486] },
  icons:{ x:1421, y:22 },
  arc:{ lx:529,ly:221, px:941,py:162, rx:1353,ry:221 },
};
...
class CasaLuna extends HTMLElement {
  ...
  _styles() {
    ...
    return `
      ...
      .detail { position:absolute; left:296px; top:140px;
        width:1112px; height:481px;
  ...
  _build() {
    ...
    const header = `
    <div style="position:absolute;left:0;top:0;width:${VB_W}px;height:151px;pointer-events:none">
      ...
      <!-- Clock + date -->
      <div id="hTime" style="position:absolute;left:56px;top:32px;font-size:44px;font-weight:800;color:#eaf4ff;letter-spacing:-1px;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif">--:--</div>
      <div id="hDay"  style="position:absolute;left:59px;top:91px;font-size:16px;font-weight:700;color:#a8cae6;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif"></div>
      <div id="hDate" style="position:absolute;left:59px;top:112px;font-size:15px;color:#7fa3c4;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif"></div>
      <div id="timeHotspot" style="position:absolute;left:51px;top:26px;width:192px;height:112px;cursor:pointer;pointer-events:auto"></div>
      <div id="weatherHotspot" style="position:absolute;left:237px;top:48px;width:230px;height:86px;cursor:pointer;pointer-events:auto"></div>
      <!-- Weather gadget — shifted left to x=190 -->
      <div id="hWxArt" style="position:absolute;left:243px;top:60px;width:64px;height:65px"></div>
      <div id="hTemp" style="position:absolute;left:323px;top:50px;font-size:32px;font-weight:700;color:#eaf4ff;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif">--°</div>
      <div id="hCond" style="position:absolute;left:323px;top:89px;font-size:14px;font-weight:600;color:#a8cae6;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif"></div>
      <div id="hWind" style="position:absolute;left:323px;top:112px;font-size:14px;color:#a8cae6;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif"></div>
      ...
      <div style="position:absolute;left:0;top:0;width:${VB_W}px;text-align:center;pointer-events:none">
        <div style="margin-top:8px;line-height:1;font-family:'Bahnschrift','Arial Narrow','Franklin Gothic Medium','Segoe UI',Impact,sans-serif;font-size:70px;font-weight:900;
          ...
          text-shadow:none;
          filter:drop-shadow(0 0 18px rgba(58,123,255,0.55)) drop-shadow(0 0 6px rgba(128,179,255,0.7))">${esc(headerTitle(c))}</div>
    ...
    <svg id="arcLayer" viewBox="0 0 630 260" preserveAspectRatio="xMidYMid meet"
      style="position:absolute;left:323px;top:48px;width:1361px;height:281px;overflow:visible;background:none">
      <defs>
    ...
    */
    const A_Y    = SL.r_stats[1] + 41 + 3 * 50 + 10 + 22;  // scaled base
    ...
    const BATT_Y = A_Y + 11;    // battery flow line pulled down
    const POLE_Y = A_Y + 49;    // grid flow line (pulled down)
    ...
    const BATT_D_X   = 1056;                    // sx(825) battery→house junction
    const BATT_D_LEN = (BATT_B_X - CR) - BATT_D_X;       // 120px toward house
    ...
    const POLE_P_X   = 384 + Math.round(5 * H_CYC) - 51; // scaled grid flow start
    ...
    const POLE_D_X   = 727;                    // sx(568) grid→house junction
    const POLE_D_LEN = POLE_D_X - (POLE_Q_X + CR);       // 125px toward house
...
console.info(`%c CASA-LUNA-16X9 %c v${VERSION} b${BUILD} `, 'background:#0a2a55;color:#7fd4ff;font-weight:700', 'background:#123;color:#9ae63c');
})();
