...
echo "Tablet is YAML mode — edit lovelace/dashboards/flux_ui_tablet.yaml (copied to /config/dashboards/)."
echo "Normal deploy does not regenerate tablet from Python (use --rebuild-tablet-yaml to regenerate)."
echo "If UI looks stale, hard-refresh (Cmd+Shift+R) or reset HA app cache."
echo "First YAML switch may need one HA restart."
