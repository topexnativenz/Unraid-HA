...
def _open_player_action(zone: str) -> dict:
    """Select zone then open music popup (tap/click on mini player)."""
    escaped = zone.replace("'", "\\'")
    return {
        "action": "custom-js-action",
        "code": (
            "[[[ "
            f"hass.callService('script', 'flux_ui_select_media_zone', {{ zone: '{escaped}' }}); "
...
def _dynamic_player_entity_template(players: list[dict]) -> str:
    default_entity = players[0]["entity"]
    zone_map = ", ".join(f"'{p['name']}': '{p['entity']}'" for p in players)
    return (
        "{% set zone_entity = {"
        + zone_map
        + "} %}"
...
def _dynamic_popup_player(players: list[dict], *, use_mediocre: bool) -> dict:
    """Single player card bound to input_select — artwork updates when zone changes."""
    entity_tpl = _dynamic_player_entity_template(players)
    if use_mediocre:
        return {
            "type": "custom:mediocre-massive-media-player-card",
            "entity_id": entity_tpl,
            "mode": "panel",
            "use_art_colors": True,
            "options": {
...
def build_music_player_popup(cfg: dict, *, use_mediocre: bool = True) -> dict | None:
    ...
    popup_cards: list[dict] = [
        ...
        _dynamic_popup_player(players, use_mediocre=use_mediocre),
    ]
